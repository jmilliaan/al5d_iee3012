...
class RRT():
    ...
    def step_plan(self):
        ...
        #find a nearest node
        nearest_index, nearest_sample = self.find_nearest(self.random_sample, 
                                                          np.array(self.samples))

        #extend to the random state
        self.next_states = self.extend(nearest_index[0], 
                                       nearest_sample.flatten(), 
                                       self.random_sample.flatten() )
        
        #check distance with goal state
        nearest_index, nearest_sample = self.find_nearest(self.goal_state, 
                                                          np.array(self.samples))        
        clear_output()
        ...
        self.next_states_goal = self.extend(nearest_index[0], 
                                            nearest_sample.flatten(), 
                                            self.goal_state.flatten())
        #print(next_states)
        if len(self.next_states_goal) == 0: 
            return False
